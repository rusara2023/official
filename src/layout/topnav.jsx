...
export default function TopNav() {
  ...
  // const [lang, setLang] = React.useState("SI");
      ...
      {({isEng, handleLan}) => (
        <Navbar className="sticky mx-auto max-w-screen-xl py-2 px-4 lg:px-8 lg:py-4 rounded-none bg-neutral-950 border-0 z-50">
          <div className="container mx-auto flex items-center justify-evenly">
            <div className="hidden lg:block w-24 text-center"></div>
            ...
            <div className="ml-8 lg:ml-0 grid grid-cols-2 w-24 text-center">
              <p
                onClick={handleLan}
                className={`cursor-pointer px-2.5 pb-1 pt-1.5 rounded ${!isEng ? "bg-white text-black" : " text-gray-600 bg-transparent"}`}
              >
                සිං
              </p>
              <p onClick={handleLan} className={`cursor-pointer px-2.5 pb-1 pt-1.5 rounded ${isEng ? "bg-white text-black" : " text-gray-600 bg-transparent"}`}>
                Eng
              </p>
            </div>
            <IconButton
